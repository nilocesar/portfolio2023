...
export const Canvas = () => {
  ...
  const base = (
    ...
    const element: ElementProps = {
      ...
      age: 0,
      wanderStep:
        config.MIN_WANDER_STEP + Math.random() * (config.MAX_WANDER_STEP - config.MIN_WANDER_STEP),
      growthRate:
        config.MIN_GROWTH_RATE + Math.random() * (config.MAX_GROWTH_RATE - config.MIN_GROWTH_RATE),
      shrinkRate:
        config.MIN_SHRINK_RATE + Math.random() * (config.MAX_SHRINK_RATE - config.MIN_SHRINK_RATE),
      ...
      update() {
        if (element.growing) {
          ...
          element.theta +=
            -element.wanderStep + Math.random() * (element.wanderStep - -element.wanderStep);
          element.x += Math.cos(element.theta) * element.growthRate * element.scale;
          element.y += Math.sin(element.theta) * element.growthRate * element.scale;
          element.scale *= element.shrinkRate;
          if (fArray.length < config.MAX_CONCURRENT && Math.random() < config.BRANCH_PROBABILITY) {
            let bEl =
  ...
  const loop = () => {
    ...
    let a, b, d;
    a = 0;
    for (b = fArray.length; a < b; a++) (d = fArray[a]), d.update(), d.render();
  ...
  }, [canvasRef, b, base, config, e, fArray, reset, startLoop, stopLoop]);
